...
sep_old <- 
  ...
  filter(u_grad > 10) %>%
  group_by(title) %>%
  summarise(total_enrollment = sum(u_grad))
...
-Define what we want to know
  -Why the course enrollment dropped from 9/1 to 9/21
  -Reason for why people dropped the course
-be thoughtful in cleaning data to see useful trends



After we have a model, Temperance suggests we should:

-Recognize that the model might not be able to fully explain all factors in a student's decision to drop the course
  -internal and external factors not accounted for may influence enrollment
